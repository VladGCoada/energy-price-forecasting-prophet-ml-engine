"""
EU ENERGY INTELLIGENCE PLATFORM - 2030 INTELLIGENCE MONOLITH
============================================================

This monolithic reference file extends your consolidated EU Energy Intelligence
Platform with a future-facing intelligence architecture:

- Lakeflow-style declarative pipeline templates
- Hybrid batch/streaming/backfill execution planning
- Market stress intelligence mart
- Forecasting layer
- Scenario simulation layer
- Recommendations layer
- Governed agent query planner
- Findings report generation

Use this as a Codex/refactor source, not as a blind replacement for your working repo.
"""

from __future__ import annotations

# =============================================================================
# ORIGINAL CONSOLIDATED PLATFORM
# =============================================================================
"""
...
# =============================================================================
# 2030 INTELLIGENCE EXTENSION
# =============================================================================

# NOTE: This extension focuses on capabilities that produce visible business
# findings. It is designed to be extracted into modular packages later.
...
ARCHITECTURE_2030_PRINCIPLES: dict[str, str] = {
    "data_product": "Every Gold/Platinum table must answer a business question.",
    "lakeflow_first": "Declarative pipelines own transformations; orchestration handles deployment/backfill.",
    "hybrid_execution": "Batch, streaming, and backfill share contracts and output schemas.",
    "explainable_ml": "Every ML output includes model version, features, confidence, and explanation.",
    "scenario_native": "The platform simulates shocks, not only reports historical data.",
    "agent_governed": "Natural-language access queries approved Gold/Platinum views only.",
    "audit_everything": "Runs, forecasts, scenarios, and recommendations are traceable.",
}


def describe_2030_architecture() -> dict[str, Any]:
    return {
        "name": "EU Energy Intelligence Platform 2030",
        "pattern": "Lakehouse Decision Intelligence Platform",
        "layers": {
            "bronze": "Immutable source-aligned raw energy data.",
            "silver": "Contracted clean time-series data.",
            "gold": "Business marts: stress, volatility, price spikes, import dependency.",
            "platinum": "Decision marts: forecasts, scenarios, recommendations.",
            "agent": "Governed natural-language interface over approved views.",
        },
        "principles": ARCHITECTURE_2030_PRINCIPLES,
        "capabilities": [
            "renewable volatility detection",
            "price spike and negative-price detection",
            "supply-demand stress scoring",
            "cross-border dependency analysis",
            "short-horizon forecasting",
            "shock scenario simulation",
            "recommendation generation",
            "governed natural-language query interface",
            "interviewer-ready findings report",
        ],
    }


LAKEFLOW_GENERATION_PIPELINE_TEMPLATE = """
# Databricks Lakeflow-style declarative pipeline template.
# Move to databricks/assets/lakeflow/generation_pipeline.py.
...
@dp.table(name="bronze_generation", comment="Raw ENTSO-E generation events.")
def bronze_generation():
    return (
        spark.readStream
            .format("cloudFiles")
            .option("cloudFiles.format", "json")
            .load("${raw_generation_path}")
            .withColumn("_ingest_ts", F.current_timestamp())
    )

@dp.table(name="silver_generation", comment="Validated renewable generation time series.")
@dp.expect_or_drop("valid_timestamp", "timestamp_utc IS NOT NULL")
@dp.expect_or_drop("valid_generation", "generation_mw IS NULL OR generation_mw >= 0")
def silver_generation():
    return (
        spark.readStream.table("LIVE.bronze_generation")
...
@dp.table(name="gold_renewable_stability", comment="Daily renewable stability mart.")
def gold_renewable_stability():
    df = spark.read.table("LIVE.silver_generation")
    return (
        df.withColumn("event_date", F.to_date("timestamp_utc"))
          .groupBy("zone", "event_date")
          .agg(
              F.sum("generation_mw").alias("total_generation_mwh"),
              F.sum(F.when(F.col("is_renewable"), F.col("generation_mw")).otherwise(F.lit(0.0))).alias("renewable_generation_mwh"),
          )
          .withColumn(
              "renewable_share_pct",
              F.when(F.col("total_generation_mwh") > 0,
                     F.col("renewable_generation_mwh") / F.col("total_generation_mwh") * 100.0)
               .otherwise(F.lit(0.0))
          )
    )
...
LAKEFLOW_MARKET_STRESS_TEMPLATE = """
# Databricks Lakeflow-style market stress pipeline template.
# Move to databricks/assets/lakeflow/market_stress_pipeline.py.
...
@dp.table(name="gold_market_stress_daily", comment="Daily market stress score.")
def gold_market_stress_daily():
    prices = spark.read.table("LIVE.silver_prices")
    generation = spark.read.table("LIVE.silver_generation")
    load = spark.read.table("LIVE.silver_load")

    price_daily = (
        prices.withColumn("event_date", F.to_date("timestamp_utc"))
              .groupBy("zone", "event_date")
              .agg(
                  ...
                  F.sum(F.when(F.col("price_z_score") > 2.0, 1).otherwise(0)).alias("price_spike_count"),
                  F.sum(F.when(F.col("price_eur_mwh") < 0, 1).otherwise(0)).alias("negative_price_count"),
              )
    )

    gen_daily = (
        generation.withColumn("event_date", F.to_date("timestamp_utc"))
                  .groupBy("zone", "event_date")
                  .agg(
                      F.sum("generation_mw").alias("total_generation_mwh"),
                      F.sum(F.when(F.col("is_renewable"), F.col("generation_mw")).otherwise(F.lit(0.0))).alias("renewable_generation_mwh"),
    ...
    load_daily = (
        load.withColumn("event_date", F.to_date("timestamp_utc"))
            .groupBy("zone", "event_date")
            .agg(
                F.avg("actual_load_mw").alias("avg_load_mw"),
                F.avg("abs_forecast_error_mw").alias("avg_forecast_error_mw"),
            )
    )

    joined = price_daily.join(gen_daily, ["zone", "event_date"], "left").join(load_daily, ["zone", "event_date"], "left").fillna(0.0)

    return (
        joined
        .withColumn("renewable_risk", F.greatest(F.lit(0.0), (F.lit(60.0) - F.col("renewable_share_pct")) / F.lit(60.0)))
        .withColumn("price_risk", F.least(F.lit(1.0), F.col("price_spike_count") / F.lit(8.0)))
        .withColumn("load_risk", F.least(F.lit(1.0), F.col("avg_forecast_error_mw") / F.lit(1000.0)))
        .withColumn("stress_score", F.round(F.lit(0.45) * F.col("price_risk") + F.lit(0.35) * F.col("renewable_risk") + F.lit(0.20) * F.col("load_risk"), 4))
        .withColumn("stress_label", F.when(F.col("stress_score") >= 0.75, "CRITICAL").when(F.col("stress_score") >= 0.50, "HIGH").when(F.col("stress_score") >= 0.25, "MEDIUM").otherwise("LOW"))
        .withColumn("main_driver", F.when((F.col("price_risk") >= F.col("renewable_risk")) & (F.col("price_risk") >= F.col("load_risk")), "PRICE_SPIKES").when((F.col("renewable_risk") >= F.col("price_risk")) & (F.col("renewable_risk") >= F.col("load_risk")), "LOW_RENEWABLE_SHARE").otherwise("LOAD_FORECAST_ERROR"))
        .withColumn("record_created_ts", F.current_timestamp())
    )
"""


def write_lakeflow_templates(base_dir: str = "databricks/assets/lakeflow") -> list[str]:
    target = Path(base_dir)
    target.mkdir(parents=True, exist_ok=True)
    files = {
        "generation_pipeline.py": LAKEFLOW_GENERATION_PIPELINE_TEMPLATE,
        "market_stress_pipeline.py": LAKEFLOW_MARKET_STRESS_TEMPLATE,
    }
    written = []
...
class HybridExecutionPlan:
    ...
    checkpoint_policy: str = "per_dataset_zone"
    expected_latency_minutes: int = 15
    notes: list[str] = field(default_factory=list)


class HybridExecutionPlanner:
    def plan_daily_batch(self, zones: list[str], datasets: Optional[list[str]] = None) -> HybridExecutionPlan:
        return HybridExecutionPlan("emit_daily_batch", ExecutionMode.BATCH, datasets or ["generation", "prices", "load", "flows"], zones, expected_latency_minutes=60, notes=["Daily refresh for complete previous-day data.", "Use checkpoints to avoid duplicate ingestion.", "Bronze append-only, Silver merge, Gold date-partition overwrite."])

    def plan_streaming_refresh(self, zones: list[str], datasets: Optional[list[str]] = None) -> HybridExecutionPlan:
        return HybridExecutionPlan("emit_near_real_time_refresh", ExecutionMode.STREAMING, datasets or ["generation", "prices", "load"], zones, expected_latency_minutes=15, notes=["Use Auto Loader/readStream for landing-zone files.", "Use watermarking on timestamp_utc.", "Output schemas match batch output schemas."])

    def plan_backfill(self, zones: list[str], start: str, end: str, datasets: Optional[list[str]] = None) -> HybridExecutionPlan:
        return HybridExecutionPlan("emit_historical_backfill", ExecutionMode.BACKFILL, datasets or ["generation", "prices", "load", "flows"], zones, period_start=start, period_end=end, expected_latency_minutes=240, notes=["Backfill should be idempotent.", "Use deterministic partition overwrite by event_date.", "Compare manifest row counts after completion."])


@dataclass
class MarketStressRecord:
    country_code: str
    event_date: str
    ...
    renewable_volatility_index: float
    avg_load_mw: float
    avg_forecast_error_mw: float
    stress_score: float
    stress_label: str
    main_driver: str
    finding_summary: str
...
class MarketStressScorer:
    def __init__(self, price_spike_threshold_count: int = 8, renewable_target_pct: float = 60.0, load_error_reference_mw: float = 1000.0) -> None:
        self.price_spike_threshold_count = price_spike_threshold_count
        self.renewable_target_pct = renewable_target_pct
        self.load_error_reference_mw = load_error_reference_mw

    @staticmethod
    def _clip(value: float, lo: float = 0.0, hi: float = 1.0) -> float:
        return max(lo, min(hi, value))

    def score_row(self, row: dict[str, Any]) -> MarketStressRecord:
        country = str(row.get("country_code") or row.get("zone") or "UNKNOWN")
        event_date = str(row.get("event_date") or row.get("summary_date") or "UNKNOWN_DATE")
        avg_price = float(row.get("avg_price_eur_mwh") or row.get("price_avg") or 0.0)
        max_price = float(row.get("max_price_eur_mwh") or row.get("price_high") or avg_price)
        price_vol = float(row.get("price_volatility") or row.get("price_stddev") or 0.0)
        spike_count = int(row.get("price_spike_count") or 0)
        negative_count = int(row.get("negative_price_count") or 0)
        renewable_share = float(row.get("renewable_share_pct") or row.get("renewable_share_avg_pct") or 0.0)
        renewable_volatility = float(row.get("renewable_volatility_index") or row.get("volatility_index") or 0.0)
        avg_load = float(row.get("avg_load_mw") or row.get("total_load_mwh") or 0.0)
        forecast_error = float(row.get("avg_forecast_error_mw") or row.get("forecast_error_avg_mw") or 0.0)

        price_risk = self._clip(spike_count / max(1, self.price_spike_threshold_count))
        price_vol_risk = self._clip(price_vol / 100.0)
        renewable_risk = self._clip((self.renewable_target_pct - renewable_share) / self.renewable_target_pct)
        renewable_vol_risk = self._clip(renewable_volatility)
        load_risk = self._clip(forecast_error / self.load_error_reference_mw)
        stress_score = round(0.30 * price_risk + 0.20 * price_vol_risk + 0.25 * renewable_risk + 0.10 * renewable_vol_risk + 0.15 * load_risk, 4)
        label = "CRITICAL" if stress_score >= 0.75 else "HIGH" if stress_score >= 0.50 else "MEDIUM" if stress_score >= 0.25 else "LOW"
        drivers = {"PRICE_SPIKES": price_risk, "PRICE_VOLATILITY": price_vol_risk, "LOW_RENEWABLE_SHARE": renewable_risk, "RENEWABLE_VOLATILITY": renewable_vol_risk, "LOAD_FORECAST_ERROR": load_risk}
        main_driver = max(drivers, key=drivers.get)
        finding = f"{country} on {event_date} had {label.lower()} market stress (score={stress_score:.2f}). Main driver: {main_driver}. Avg price={avg_price:.2f} EUR/MWh, max price={max_price:.2f} EUR/MWh, renewable share={renewable_share:.1f}%, price spikes={spike_count}, negative-price intervals={negative_count}."
        return MarketStressRecord(country, event_date, avg_price, max_price, price_vol, spike_count, negative_count, renewable_share, renewable_volatility, avg_load, forecast_error, stress_score, label, main_driver, finding)

    def score_records(self, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return [self.score_row(row).__dict__ for row in rows]


def build_market_stress_from_local_records(price_rows: list[dict[str, Any]], generation_daily_rows: list[dict[str, Any]], load_rows: Optional[list[dict[str, Any]]] = None) -> list[dict[str, Any]]:
    grouped_prices: dict[tuple[str, str], list[float]] = {}
    grouped_negatives: dict[tuple[str, str], int] = {}
    grouped_spikes: dict[tuple[str, str], int] = {}
    for row in price_rows:
        country = str(row.get("country_code") or row.get("zone") or "UNKNOWN")
        event_date = str(row.get("event_date") or str(row.get("event_timestamp_utc") or "")[:10] or "UNKNOWN_DATE")
        key = (country, event_date)
        price = row.get("price_eur_mwh")
        if isinstance(price, (int, float)):
            grouped_prices.setdefault(key, []).append(float(price))
            grouped_negatives[key] = grouped_negatives.get(key, 0) + (1 if price < 0 else 0)
    for key, values in grouped_prices.items():
        if len(values) < 2:
            grouped_spikes[key] = 0
            continue
        avg = statistics.mean(values)
        std = statistics.pstdev(values) or 1.0
        grouped_spikes[key] = sum(1 for v in values if ((v - avg) / std) > 2.0)
    gen_index: dict[tuple[str, str], dict[str, Any]] = {}
    for row in generation_daily_rows:
        country = str(row.get("country_code") or row.get("zone") or "UNKNOWN")
        event_date = str(row.get("event_date") or row.get("summary_date") or "UNKNOWN_DATE")
        gen_index[(country, event_date)] = row
    load_index: dict[tuple[str, str], dict[str, Any]] = {}
    for row in load_rows or []:
        country = str(row.get("country_code") or row.get("zone") or "UNKNOWN")
        event_date = str(row.get("event_date") or str(row.get("event_timestamp_utc") or "")[:10] or "UNKNOWN_DATE")
        load_index[(country, event_date)] = row
    combined = []
    for key, prices in grouped_prices.items():
        country, event_date = key
        gen = gen_index.get(key, {})
        load = load_index.get(key, {})
        combined.append({"country_code": country, "event_date": event_date, "avg_price_eur_mwh": statistics.mean(prices) if prices else 0.0, "max_price_eur_mwh": max(prices) if prices else 0.0, "price_volatility": statistics.pstdev(prices) if len(prices) > 1 else 0.0, "price_spike_count": grouped_spikes.get(key, 0), "negative_price_count": grouped_negatives.get(key, 0), "renewable_share_pct": float(gen.get("renewable_share_pct") or 0.0), "renewable_volatility_index": float(gen.get("volatility_index") or gen.get("renewable_volatility_index") or 0.0), "avg_load_mw": float(load.get("avg_load_mw") or load.get("actual_load_mw") or 0.0), "avg_forecast_error_mw": float(load.get("avg_forecast_error_mw") or load.get("abs_forecast_error_mw") or 0.0)})
    return MarketStressScorer().score_records(combined)
...
class SimpleRobustForecaster:
    def __init__(self, horizon_intervals: int = 96, season_lag: int = 96, rolling_window: int = 96) -> None:
    ...
    def forecast_series(self, zone: str, timestamps: list[str], values: list[float]) -> list[dict[str, Any]]:
        if not values:
            return []
        recent = values[-self.rolling_window:]
        rolling_mean = statistics.mean(recent)
        mad = self._mad(recent)
        interval_width = max(5.0, 1.4826 * mad * 1.96)
        last_ts = datetime.fromisoformat(timestamps[-1].replace("Z", "+00:00")) if timestamps else datetime.utcnow().replace(tzinfo=timezone.utc)
        if last_ts.tzinfo is None:
            last_ts = last_ts.replace(tzinfo=timezone.utc)
        out = []
        for step in range(1, self.horizon_intervals + 1):
            seasonal_idx = len(values) - self.season_lag + ((step - 1) % max(1, self.season_lag))
            seasonal_component = values[seasonal_idx] if 0 <= seasonal_idx < len(values) else rolling_mean
            yhat = 0.65 * seasonal_component + 0.35 * rolling_mean
            ts = last_ts + timedelta(minutes=15 * step)
            out.append(ForecastRecord(zone, ts.isoformat(), round(yhat, 4), round(yhat - interval_width, 4), round(yhat + interval_width, 4), "simple_robust_seasonal_forecaster", "2030-local-v1", datetime.utcnow().replace(tzinfo=timezone.utc).isoformat()).__dict__)
        return out


class ForecastRiskClassifier:
    def __init__(self, high_price_threshold: float = 200.0, negative_price_threshold: float = 0.0) -> None:
        self.high_price_threshold = high_price_threshold
        self.negative_price_threshold = negative_price_threshold

    def classify(self, forecasts: list[dict[str, Any]]) -> list[dict[str, Any]]:
        enriched = []
        for row in forecasts:
            yhat = float(row.get("yhat") or 0.0)
            upper = float(row.get("yhat_upper") or yhat)
            lower = float(row.get("yhat_lower") or yhat)
            risk = "PRICE_SPIKE_RISK" if upper >= self.high_price_threshold else "NEGATIVE_PRICE_RISK" if lower < self.negative_price_threshold else "NORMAL"
            new = dict(row)
            new["forecast_risk_label"] = risk
            new["forecast_confidence_band_width"] = round(upper - lower, 4)
            enriched.append(new)
        return enriched
...
class EnergyScenario:
    ...
    zone: str
    event_date: str
    wind_drop_pct: float = 0.0
    solar_drop_pct: float = 0.0
    load_increase_pct: float = 0.0
    import_constraint_pct: float = 0.0
    price_shock_eur_mwh: float = 0.0
...
class ScenarioSimulator:
    def simulate(self, baseline: dict[str, Any], scenario: EnergyScenario) -> dict[str, Any]:
        scorer = MarketStressScorer()
        adjusted = dict(baseline)
        renewable_share = float(adjusted.get("renewable_share_pct") or 0.0)
        avg_load = float(adjusted.get("avg_load_mw") or 0.0)
        avg_price = float(adjusted.get("avg_price_eur_mwh") or adjusted.get("price_avg") or 0.0)
        max_price = float(adjusted.get("max_price_eur_mwh") or adjusted.get("price_high") or avg_price)
        renewable_penalty = scenario.wind_drop_pct * 0.7 + scenario.solar_drop_pct * 0.3
        adjusted["renewable_share_pct"] = max(0.0, renewable_share * (1 - renewable_penalty / 100.0))
        adjusted["avg_load_mw"] = avg_load * (1 + scenario.load_increase_pct / 100.0)
        adjusted["avg_price_eur_mwh"] = avg_price + scenario.price_shock_eur_mwh
        adjusted["max_price_eur_mwh"] = max_price + scenario.price_shock_eur_mwh
        adjusted["renewable_volatility_index"] = float(adjusted.get("renewable_volatility_index") or 0.0) + scenario.import_constraint_pct / 100.0
        baseline_score = float(baseline.get("stress_score") or 0.0)
        simulated = scorer.score_row(adjusted)
        return ScenarioResult(scenario.scenario_id, scenario.zone, scenario.event_date, baseline_score, simulated.stress_score, round(simulated.stress_score - baseline_score, 4), simulated.stress_label, self.recommend(simulated), datetime.utcnow().replace(tzinfo=timezone.utc).isoformat()).__dict__

    def recommend(self, simulated: MarketStressRecord) -> str:
        if simulated.stress_label in {"CRITICAL", "HIGH"}:
            if simulated.main_driver == "LOW_RENEWABLE_SHARE":
                return "Inspect renewable shortfall, import capacity, and reserve margins."
            if simulated.main_driver == "PRICE_SPIKES":
                return "Trigger price-spike review and compare spikes against renewable dips."
            if simulated.main_driver == "LOAD_FORECAST_ERROR":
                return "Review load forecast drift and weather assumptions."
        return "No urgent action. Continue monitoring."


def generate_default_scenarios(zone: str, event_date: str) -> list[EnergyScenario]:
    return [
        EnergyScenario(str(uuid.uuid4()), "wind_drop_20pct", zone, event_date, wind_drop_pct=20.0),
        EnergyScenario(str(uuid.uuid4()), "load_increase_10pct", zone, event_date, load_increase_pct=10.0),
        EnergyScenario(str(uuid.uuid4()), "import_constraint_30pct", zone, event_date, import_constraint_pct=30.0),
        EnergyScenario(str(uuid.uuid4()), "combined_stress_case", zone, event_date, wind_drop_pct=20.0, load_increase_pct=10.0, import_constraint_pct=20.0, price_shock_eur_mwh=50.0),
    ]
...
class EnergyRecommendationEngine:
    def recommend_from_stress(self, stress_row: dict[str, Any]) -> list[dict[str, Any]]:
        zone = str(stress_row.get("country_code") or stress_row.get("zone") or "UNKNOWN")
        event_date = str(stress_row.get("event_date") or "UNKNOWN_DATE")
        label = str(stress_row.get("stress_label") or "LOW")
        driver = str(stress_row.get("main_driver") or "UNKNOWN")
        score = float(stress_row.get("stress_score") or 0.0)
        recs: list[Recommendation] = []
        if label in {"HIGH", "CRITICAL"}:
            recs.append(Recommendation(str(uuid.uuid4()), zone, event_date, label, "MARKET_STRESS", f"Investigate {driver} as the primary cause of {label.lower()} stress.", stress_row, datetime.utcnow().replace(tzinfo=timezone.utc).isoformat()))
        if float(stress_row.get("renewable_share_pct") or 0.0) < 35.0:
            recs.append(Recommendation(str(uuid.uuid4()), zone, event_date, "MEDIUM" if label == "LOW" else label, "RENEWABLE_SHORTFALL", "Review renewable shortfall versus imports, load, and price spikes.", {"renewable_share_pct": stress_row.get("renewable_share_pct"), "stress_score": score}, datetime.utcnow().replace(tzinfo=timezone.utc).isoformat()))
        if int(stress_row.get("price_spike_count") or 0) > 0:
            recs.append(Recommendation(str(uuid.uuid4()), zone, event_date, "HIGH" if label in {"HIGH", "CRITICAL"} else "MEDIUM", "PRICE_SPIKE", "Produce a price-spike explanation and compare against renewable share and load forecast error.", {"price_spike_count": stress_row.get("price_spike_count"), "max_price_eur_mwh": stress_row.get("max_price_eur_mwh")}, datetime.utcnow().replace(tzinfo=timezone.utc).isoformat()))
        return [r.__dict__ for r in recs]


ALLOWED_AGENT_TABLES = {"gold_market_stress_daily", "gold_renewable_stability", "gold_price_spike_analysis", "gold_market_summary", "platinum_carbon_adjusted_prices", "platinum_arbitrage_opportunities", "platinum_scenario_simulations", "platinum_recommendations"}
...
class GovernedEnergyAgentPlanner:
    def plan(self, question: str, catalog: str = "emit_dev", gold_schema: str = "gold", platinum_schema: str = "platinum") -> AgentQueryPlan:
        q = question.lower()
        if "stress" in q or "risk" in q:
            table = f"{catalog}.{gold_schema}.gold_market_stress_daily"
            sql = f"SELECT zone, event_date, stress_score, stress_label, main_driver, finding_summary FROM {table} ORDER BY event_date DESC, stress_score DESC LIMIT 20"
            return AgentQueryPlan(question, ["gold_market_stress_daily"], sql, ["Gold table only.", "No Bronze access."])
        if "renewable" in q:
            table = f"{catalog}.{gold_schema}.gold_renewable_stability"
            sql = f"SELECT zone, event_date, renewable_share_pct, renewable_volatility_index, renewable_dip_flag FROM {table} ORDER BY event_date DESC, renewable_volatility_index DESC LIMIT 20"
            return AgentQueryPlan(question, ["gold_renewable_stability"], sql, ["Approved renewable stability mart only."])
        if "price" in q or "spike" in q:
            table = f"{catalog}.{gold_schema}.gold_price_spike_analysis"
            sql = f"SELECT zone, timestamp_utc, price_eur_mwh, price_z_score, price_spike_flag, negative_price_flag FROM {table} WHERE price_spike_flag = true OR negative_price_flag = true ORDER BY timestamp_utc DESC LIMIT 50"
            return AgentQueryPlan(question, ["gold_price_spike_analysis"], sql, ["Approved price spike mart only."])
        table = f"{catalog}.{gold_schema}.gold_market_summary"
        return AgentQueryPlan(question, ["gold_market_summary"], f"SELECT * FROM {table} ORDER BY summary_date DESC LIMIT 20", ["Fallback approved Gold summary only."])


class FindingsReportBuilder:
    def build_market_stress_report(self, stress_rows: list[dict[str, Any]], scenario_rows: Optional[list[dict[str, Any]]] = None, recommendation_rows: Optional[list[dict[str, Any]]] = None) -> str:
        stress_rows = sorted(stress_rows, key=lambda r: (str(r.get("event_date")), float(r.get("stress_score") or 0.0)), reverse=True)
        lines = ["# EU Energy Market Intelligence Findings", "", "## Executive Summary", ""]
        if not stress_rows:
            lines.append("No market stress records available.")
            return "\n".join(lines)
        top = stress_rows[0]
        lines.append(f"The highest observed market stress was **{top.get('stress_label')}** for **{top.get('country_code') or top.get('zone')}** on **{top.get('event_date')}** with score **{float(top.get('stress_score') or 0.0):.2f}**.")
        lines.extend(["", "## Top Market Stress Days", "", "| Zone | Date | Stress | Score | Main Driver | Summary |", "|---|---:|---:|---:|---|---|"])
        for row in stress_rows[:10]:
            lines.append(f"| {row.get('country_code') or row.get('zone')} | {row.get('event_date')} | {row.get('stress_label')} | {float(row.get('stress_score') or 0.0):.2f} | {row.get('main_driver')} | {row.get('finding_summary', '')} |")
        if scenario_rows:
            lines.extend(["", "## Scenario Simulation Highlights", "", "| Scenario | Zone | Date | Baseline | Simulated | Delta | Recommendation |", "|---|---|---:|---:|---:|---:|---|"])
            for row in scenario_rows[:10]:
                lines.append(f"| {row.get('scenario_id')} | {row.get('zone')} | {row.get('event_date')} | {float(row.get('baseline_stress_score') or 0.0):.2f} | {float(row.get('simulated_stress_score') or 0.0):.2f} | {float(row.get('stress_delta') or 0.0):.2f} | {row.get('recommendation', '')} |")
        if recommendation_rows:
            lines.extend(["", "## Recommendations", ""])
            for rec in recommendation_rows[:10]:
                lines.append(f"- **{rec.get('severity')} / {rec.get('category')}** ({rec.get('zone')} {rec.get('event_date')}): {rec.get('recommendation')}")
        lines.extend(["", "## Why This Matters", "", "This report demonstrates that the project is not merely an ETL pipeline. It produces interpretable energy-market intelligence: stress scoring, scenario simulation, and explainable recommendations."])
        return "\n".join(lines)


class Local2030IntelligenceRunner:
    def __init__(self, processed_base_dir: str = "data/processed") -> None:
        self.base = Path(processed_base_dir)

    def _read_json_records(self, relative_path: str) -> list[dict[str, Any]]:
        path = self.base / relative_path
        if not path.exists():
            return []
        return json.loads(path.read_text(encoding="utf-8"))

    def _write_json_records(self, relative_path: str, rows: list[dict[str, Any]]) -> str:
        path = self.base / relative_path
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps(rows, indent=2, default=str), encoding="utf-8")
        return str(path)

    def run_market_stress_from_existing_outputs(self) -> dict[str, Any]:
        generation_daily = self._read_json_records("gold/renewable_stability/records.json")
        prices = self._read_json_records("silver/prices/records.json")
        loads = self._read_json_records("silver/load/records.json")
        if not prices:
            prices = []
            for row in generation_daily:
                country = row.get("country_code") or row.get("zone") or "UNKNOWN"
                event_date = row.get("event_date") or "UNKNOWN_DATE"
                renewable_share = float(row.get("renewable_share_pct") or 0.0)
                synthetic_price = 120.0 - renewable_share
                prices.append({"country_code": country, "event_date": event_date, "price_eur_mwh": synthetic_price})
        stress_rows = build_market_stress_from_local_records(prices, generation_daily, loads)
        stress_path = self._write_json_records("gold/market_stress_daily/records.json", stress_rows)
        scenarios = []
        recommendations = []
        rec_engine = EnergyRecommendationEngine()
        sim = ScenarioSimulator()
        for stress in stress_rows:
            zone = str(stress.get("country_code") or stress.get("zone") or "UNKNOWN")
            event_date = str(stress.get("event_date") or "UNKNOWN_DATE")
            for sc in generate_default_scenarios(zone, event_date):
                scenarios.append(sim.simulate(stress, sc))
            recommendations.extend(rec_engine.recommend_from_stress(stress))
        scenarios_path = self._write_json_records("platinum/scenario_simulations/records.json", scenarios)
        rec_path = self._write_json_records("platinum/recommendations/records.json", recommendations)
        report = FindingsReportBuilder().build_market_stress_report(stress_rows, scenarios, recommendations)
        report_path = self.base / "reports" / "market_intelligence_findings.md"
        report_path.parent.mkdir(parents=True, exist_ok=True)
        report_path.write_text(report, encoding="utf-8")
        return {"stress_rows": len(stress_rows), "scenario_rows": len(scenarios), "recommendation_rows": len(recommendations), "stress_path": stress_path, "scenarios_path": scenarios_path, "recommendations_path": rec_path, "report_path": str(report_path)}


def run_2030_command(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser("emit-2030")
    sub = parser.add_subparsers(dest="cmd", required=True)
    sub.add_parser("describe-2030")
    sub.add_parser("write-lakeflow")
    p_local = sub.add_parser("run-local-intelligence")
    p_local.add_argument("--processed-base-dir", default="data/processed")
    p_agent = sub.add_parser("agent-plan")
    p_agent.add_argument("question")
    args = parser.parse_args(argv)
    if args.cmd == "describe-2030":
        print(json.dumps(describe_2030_architecture(), indent=2))
        return 0
    if args.cmd == "write-lakeflow":
        print(json.dumps({"written": write_lakeflow_templates()}, indent=2))
        return 0
    if args.cmd == "run-local-intelligence":
        print(json.dumps(Local2030IntelligenceRunner(args.processed_base_dir).run_market_stress_from_existing_outputs(), indent=2))
        return 0
    if args.cmd == "agent-plan":
        print(json.dumps(GovernedEnergyAgentPlanner().plan(args.question).__dict__, indent=2))
        return 0
    return 1


def test_market_stress_scorer_low_renewable_driver():
    row = {"country_code": "NL", "event_date": "2024-01-01", "avg_price_eur_mwh": 100.0, "max_price_eur_mwh": 180.0, "price_volatility": 20.0, "price_spike_count": 0, "negative_price_count": 0, "renewable_share_pct": 10.0, "renewable_volatility_index": 0.1, "avg_load_mw": 12000.0, "avg_forecast_error_mw": 100.0}
    result = MarketStressScorer().score_row(row)
    assert result.main_driver == "LOW_RENEWABLE_SHARE"
    assert result.stress_score > 0.0


def test_scenario_simulator_increases_stress():
    baseline = {"country_code": "NL", "event_date": "2024-01-01", "avg_price_eur_mwh": 50.0, "max_price_eur_mwh": 100.0, "price_volatility": 10.0, "price_spike_count": 0, "negative_price_count": 0, "renewable_share_pct": 50.0, "renewable_volatility_index": 0.1, "avg_load_mw": 10000.0, "avg_forecast_error_mw": 100.0, "stress_score": 0.2}
    scenario = EnergyScenario(str(uuid.uuid4()), "combined", "NL", "2024-01-01", wind_drop_pct=30, load_increase_pct=20, price_shock_eur_mwh=100)
    result = ScenarioSimulator().simulate(baseline, scenario)
    assert result["simulated_stress_score"] >= result["baseline_stress_score"]


def test_agent_planner_uses_allowed_tables():
...
def test_simple_forecaster_outputs_horizon():
    timestamps = [f"2024-01-01T{i:02d}:00:00+00:00" for i in range(24)]
    values = [50.0 + i for i in range(24)]
    forecasts = SimpleRobustForecaster(horizon_intervals=4, season_lag=4, rolling_window=8).forecast_series("NL", timestamps, values)
    assert len(forecasts) == 4
    assert forecasts[0]["zone"] == "NL"


if __name__ == "__main__":
    _known_2030 = {"describe-2030", "write-lakeflow", "run-local-intelligence", "agent-plan"}
    if len(sys.argv) > 1 and sys.argv[1] in _known_2030:
        raise SystemExit(run_2030_command(sys.argv[1:]))
